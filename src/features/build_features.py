...
def build_feature_registry() -> list[dict[str, object]]:
    ...
    return [
        ...
        {
            "feature": "distance_to_metro_route_km",
            "source": "openrouteservice Directions API, profile foot-walking",
            "type": "numeric",
            "offline": True,
            "online": True,
            "refresh": "weekly snapshot / API route cache",
            "description": "Pedestrian route distance from listing coordinates to the named metro station. Falls back to missing when API key or route is unavailable.",
            "leakage_risk": "low",
        },
        {
            "feature": "duration_to_metro_route_min",
            "source": "openrouteservice Directions API, profile foot-walking",
            "type": "numeric",
            "offline": True,
            "online": True,
            "refresh": "weekly snapshot / API route cache",
            "description": "Estimated walking duration from listing coordinates to the named metro station.",
            "leakage_risk": "low",
        },
        {
            "feature": "metro_route_provider",
            "source": "routing metadata",
            "type": "categorical metadata",
            "offline": True,
            "online": True,
            "refresh": "weekly snapshot / API route cache",
            "description": "Routing source for metro walking distance, e.g. openrouteservice:foot-walking.",
            "leakage_risk": "low",
        },
        {
            "feature": "metro_known",
...
def main() -> None:
    ...
    if not args.input.exists():
        raise FileNotFoundError(
            ...
            "Run `python -m src.features.geocoder` first to create the geocoded clean dataset."
        )
