...
| **Geocode** | **clean dataset** | **clean dataset + coordinates, haversine distances, optional metro walking route distance** | **`src/features/geocoder.py`** |
...
| Geocoded dataset | 1300 rows x 36 columns |
| Offline feature table | 1300 rows x 50 columns |
...
| `distance_to_metro_route_km` | openrouteservice foot-walking route distance | weekly / request with API key | yes | yes |
| `duration_to_metro_route_min` | openrouteservice foot-walking route duration | weekly / request with API key | yes | yes |
| `metro_known` | flag | weekly / request | yes | yes |
| `residential_complex` | CIAN listing | weekly / request | yes | cautiously |

Route-distance note: `distance_to_metro_km` is always available as the
reproducible straight-line fallback. `distance_to_metro_route_km` and
`duration_to_metro_route_min` are filled only when `OPENROUTESERVICE_API_KEY`
is set and the pipeline is run with `--with-routing`.
