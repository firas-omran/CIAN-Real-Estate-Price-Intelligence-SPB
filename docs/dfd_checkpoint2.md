...
flowchart LR
    ...
    ORS[External Entity: openrouteservice] -->|foot-walking route distance| P5
    P5 -->|persisted lookups| D7[(Geocode + Route Cache + Metro / District Reference)]
    D7 --> P5
    P5 -->|listings with lat, lon, haversine + route distances| D3G[(Data Store: Geocoded Dataset)]
...
| Geocoded Dataset | clean listings enriched with `lat, lon, geo_precision, distance_to_center_km, distance_to_metro_km, distance_to_metro_route_km, duration_to_metro_route_min, metro_known` |
| Geocode + Route Cache + Metro / District Reference | persisted `data/cache/geocode_cache.json`, `data/cache/route_cache.json`, `data/reference/metro_spb_coords.json`, `data/reference/spb_district_centroids.json` — also reused at serving time so the prediction API does not depend on repeated live geocoding/routing calls for known addresses |
